...
export default function HeartIcon({ id, product }) {
  ...
  const { wishIndex, setWishIndex } = useContext(context.UserContext);
  const [isLoved, setIsLoved] = useState(false);
  const list = wishIndex;

  const addToWishList = function () {
    if (!wishIndex.includes(id)) {
      list.push(id);
      setWishIndex(list);
    } else {
      list.splice(wishIndex.indexOf(id), 1);
      setWishIndex(list);
    }
  };

  useEffect(() => {
    setIsLoved(wishIndex.includes(id));
  }, []);
  return (
    <svg
      onClick={() => {
